...
function Main( $scriptDir )
{
    ...
    ### Uninstall Hive
    ###
    Uninstall "oozie" $nodeInstallRoot
    ###
    ### Delete the oozie data directory
    ###
    Write-Log "Deleting oozie data directory"
    if ( (Test-Path ENV:OOZIE_DATA) -and ($ENV:OOZIE_DATA) -and (Test-Path "${ENV:OOZIE_DATA}\*") )
    {
    ...
    Write-Log "Successfully uninstalled Oozie."
